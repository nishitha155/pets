...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchTotalUsers = async () => {
      try {
        const responseu = await fetch('https://pets-2.onrender.com/api/user/total');
        if (!responseu.ok) {
          throw new Error(`Error: ${responseu.statusText}`);
        }
        const responsese = await fetch(https://pets-2.onrender.com/api/sellerss/total');
        if (!responsese.ok) {
          throw new Error(`Error: ${responsese.statusText}`);
        }
        const responsep = await fetch('https://pets-2.onrender.com/api/productss/total');
        if (!responsep.ok) {
          throw new Error(`Error: ${responsep.statusText}`);
        }
        const responsec = await fetch('https://pets-2.onrender.com/api/complaintss/total');
        if (!responsec.ok) {
          throw new Error(`Error: ${responsec.statusText}`);
        }
        const responsesa = await fetch('https://pets-2.onrender.com/api/salonss/total');
        if (!responsesa.ok) {
