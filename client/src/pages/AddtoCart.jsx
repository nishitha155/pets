...
const AddtoCart = () => {
  ...
  const removeFromCart = async (title) => {
    try {
      const response = await fetch(`https://pets-2.onrender.com/api/cart/${userid}/${title}`, {
        method: 'DELETE',
      });
      ...
      <div className="mainproduct1">
        {cart.products.map((item) => (
          <div key={item.id} className="ppcard">
            <Image  src={`https://pets-2.onrender.com/uploads/${item.image}`} alt={item.title} objectFit="cover" boxSize="20vw" />
